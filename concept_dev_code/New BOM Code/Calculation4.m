function [BOM_toEarth] = Calculation4(a_1,a_2,t_T,total_mass_fraction,M_d,r_tank,number_of_tanks,str)
...
disp('--------')
BOM_toEarth = BOM_duration;


time =  1:((t_T*years_to_secs)/length(distance_from_sun)):t_T*years_to_secs;
figure()
plot(flip(distance_from_sun)./a_u,flip(Boil_off_rate))
title('Traveling Back to Earth')
ylabel('Boil off Mass Rate (kg/s)')
xlabel('Distance (AU)')

figure()
plot(time./years_to_secs,flip(BOM))
title('Traveling Back to Earth')
